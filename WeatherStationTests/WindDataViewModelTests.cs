...
namespace WeatherStationTests
{
    public class WindDataViewModelTests : IDisposable
    {
        ...
        public void KPHtoMPS_AlwaysReturnGoodValue(double kph, double expected)
        {
            ...
            // Act       
            double actual = _sut.KPHtoMPS(kph);
        ...
        {
            // Arrange
            var actual = _sut.GetDataCommand;

            // Act       

            // Assert
            Assert.Throws<NullReferenceException>(() => actual.Execute(_sut.WindDataService));
        ...
        {
            // Arrange
            var GetDataCommand = _sut.GetDataCommand;
            Mock<IWindDataService> _myMock = new Mock<IWindDataService>();
            _myMock.Setup(x => x.GetDataAsync()).Returns(GetSomeDataModel());

            // Act
            _sut.CurrantData = await _myMock.Object.GetDataAsync();
            var actual = _sut.CurrantData;
